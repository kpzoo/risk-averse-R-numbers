...
function [Iday, Lam, Rtrue, tday, Iwarn, distvals] = epiSimScenDemeRand(scenNo, nday, distNo, simVals)

% Assumptions and notes
% - during offset add random infections
...
scenNam = {'control', 'square-wave', 'cascade', 'boom-bust', 'filtered', 'waves', ...
    'noise valley', 'boom-bust-boom', 'fixed'};
...
%     mu = Rtrue(i)*Lam(i); r = 0.5;
%     p = mu/(mu + r); Iday(i) = nbinrnd(r, 1-p);
end
...
% Random offsets with mean of 5 infections (imports)
Ioff = round(3*rand(1, simVals.offset));

% Add simple offset of zeros
idoff = 1:(length(Iday) - simVals.offset);
Iday = [Ioff Iday(idoff)];
Rtrue = [ones(1, simVals.offset) Rtrue(idoff)];
Lam = [Ioff Lam(idoff)];
